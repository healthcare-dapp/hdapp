...
import { NotificationItem } from "../../managers/notifications.manager";
import { sessionManager } from "../../managers/session.manager";
import { trimWeb3Address } from "../../utils/trim-web3-address";

export const NotificationsList = observer<{ notifications: NotificationItem[] }>(x => {
    const theme = useTheme();
    return (
        <List sx={{ width: "100%", flex: 1, minHeight: 400 }}>
            { x.notifications.length === 0 && (
            ...
            { x.notifications.map((n, i) => {
                ...
                const title = n.type === "connection_established"
                    ...
                    : n.type === "record_permissions_granted"
                        ? "Person granted access to their data"
                        : n.type === "user_connection_created"
                            ? `${trimWeb3Address(n.userAddress)} accepted your connection request`
                            : n.type === "user_connection_requested"
                                ? "A person wants to connect with you"
                            // @ts-ignore
                                : n.type;

                const description = n.type === "connection_established"
                    ? `Device ${n.deviceName} of ${n.user} has established a peer-to-peer connection with you.`
                    : n.type === "record_permissions_granted"
                        ? `Person ${n.ownerName} has shared with you their medical record.`
                        : n.type === "user_connection_created"
                            ? "You can now exchange medical data with them and they can request medical data from you."
                            : n.type === "user_connection_requested"
                                ? `Person with address ${trimWeb3Address(n.userAddress)} wants to connect with you. Only accept connection requests from people you know.`
                            // @ts-ignore
                                : n.type;
                return (
